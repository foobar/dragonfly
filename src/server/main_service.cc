...
DispatchResult Service::DispatchCommand(
  ...
  DFLY_TRACY_DISPATCH_ZONE(kDispatchCommand);
  ...
  if (parsed_cmd->mc_command()) {
    {
      DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchResolve);
  ...
  if (cid == nullptr) {
    DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchUnknownCommand);
    invoke_pre_dispatch();
  ...
  if (dfly_cntx->async_dispatch && cid->IsBlocking()) {
    DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchBlockingFlush);
  ...
  if (auto* conn = cmd_cntx->conn(); conn /* replica context doesn't have an owner */) {
    DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchPauseCheck);
    if (VLOG_IS_ON(2)) {
  ...
  {
    DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchVerify);
    if (auto err = VerifyCommandState(*cid, args_no_cmd, *dfly_cntx); err) {
      DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchVerifyFailure);
      LOG_IF(WARNING,
  ...
  if (dfly_cntx->conn_state.exec_info.IsCollecting() && !is_trans_cmd) {
    DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchMultiQueue);
  ...
  {
    DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchTransactionAndInvoke);
    ...
    {
      DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchInvoke);
      res = InvokeCmd(cmd_cntx->tail_args(), cmd_cntx);
    }
    if (dispatched_tx) {
      DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchTransactionComplete);
    ...
    if ((res != DispatchResult::OK) && (res != DispatchResult::OOM)) {
      DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kDispatchErrorClose);
...
DispatchResult Service::InvokeCmd(const facade::ParsedArgs& tail_args, CommandContext* cmd_cntx) {
  ...
  DispatchResult res = DispatchResult::OK;
  try {
    DFLY_TRACY_DISPATCH_ZONE(kInvokeCmdHandler);
...
uint32_t Service::DispatchSquashedBatch(
    ...
    absl::FunctionRef<void(facade::ParsedCommand*)>* pre_dispatch_cb) {
  DFLY_TRACY_DISPATCH_ZONE(kSquashDispatchBatch);
  ...
  auto perform_squash = [&] {
    ...
    {
      DFLY_TRACY_DISPATCH_ZONE(kSquashDispatchTransactionSetup);
      if (!dist_trans) {
    ...
    {
      DFLY_TRACY_DISPATCH_ZONE(kSquashDispatchExecute);
      stats += MultiCommandSquasher::Execute(std::move(cmd_gen), rb, dfly_cntx, this, opts);
    }
    {
      DFLY_TRACY_DISPATCH_ZONE(kSquashDispatchTransactionTeardown);
      dfly_cntx->transaction = nullptr;
    }
  ...
  for (unsigned i = 0; i < count && cmd; i++) {
    DFLY_TRACY_DISPATCH_ZONE(kSquashDispatchCommand);
    ...
    {
      DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kSquashDispatchResolve);
    ...
    if (pre_dispatch_cb) {
      DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kSquashDispatchPreDispatch);
      (*pre_dispatch_cb)(cmd);
    }
    {
      DFLY_TRACY_DISPATCH_FORENSIC_ZONE(kSquashDispatchVerify);
  ...
  if (batch_has_write_command && tl_write_connection_throttling_sleep_usec > 0) {
    DFLY_TRACY_DISPATCH_ZONE(kSquashDispatchThrottleSleep);
  ...
  if (dist_trans) {
    DFLY_TRACY_DISPATCH_ZONE(kSquashDispatchUnlock);
