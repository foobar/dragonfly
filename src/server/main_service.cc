...
uint32_t Service::DispatchSquashedBatch(
    ...
    absl::FunctionRef<void(facade::ParsedCommand*)>* pre_dispatch_cb) {
  DFLY_TRACY_DISPATCH_ZONE("Squash.DispatchBatch");
  ...
  auto perform_squash = [&] {
    ...
    {
      DFLY_TRACY_DISPATCH_ZONE("Squash.Dispatch.TransactionSetup");
      if (!dist_trans) {
    ...
    {
      DFLY_TRACY_DISPATCH_ZONE("Squash.Dispatch.Execute");
      stats += MultiCommandSquasher::Execute(std::move(cmd_gen), rb, dfly_cntx, this, opts);
    }
    {
      DFLY_TRACY_DISPATCH_ZONE("Squash.Dispatch.TransactionTeardown");
      dfly_cntx->transaction = nullptr;
    }
  ...
  if (batch_has_write_command && tl_write_connection_throttling_sleep_usec > 0) {
    DFLY_TRACY_DISPATCH_ZONE("Squash.Dispatch.ThrottleSleep");
  ...
  if (dist_trans) {
    DFLY_TRACY_DISPATCH_ZONE("Squash.Dispatch.Unlock");
